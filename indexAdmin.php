...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  { 
    ...
    echo "<td><a href='?curdir=$curdir&newdir=$value' title='Descendre'><img height=15 src='admin/dossier.png'></a> </td>";
    echo "<td>". $value . "</td>" ;
  ...
  { if( is_dir($curdir.$value) )	continue; // SEULEMENT FILE
	...
	echo "<td align=center><a href='?CMD=SHOW&name=$value'><img height=15 src='admin/fichier.png'></td>" ;
	echo "<td>". $value . "</td>" ;
...
function CMD_MKFILE(){
 ...
 $cpt = 1 ;
  ...
  do
  {
    $home = $_SESSION['home'];
    $pwd = $_SESSION['pwd'];
    $current = $home.$pwd ; // Repertoire courant
    $name ='file'.$cpt.".txt" ; // Nouveau nom
    $path = $current.$name ; // Chemin du nouveau dir
    if( ! file_exists ($path) ) // S'il n'existe pas déjà
      {
        $handle = fopen($path,"w");
        fclose($handle);
...
//
    ...
    {	
        $_SESSION['home'] = 'C:\wamp64\www\SL_Elec_plomb';
	    $_SESSION['pwd' ] = '/' ;
    }
